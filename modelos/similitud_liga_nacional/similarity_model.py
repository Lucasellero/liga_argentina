...
def _weighted_vectors(norm_df: pd.DataFrame) -> np.ndarray:
    ...
    # Multiplicar por sqrt(w) porque cosine(a,b) = dot(a,b)/(|a||b|)
    # y queremos que cada dimensión contribuya proporcionalmente a su peso.
...
class SimilarityModel:
    """
    Modelo de similitud entre jugadores de Liga Nacional.

    Uso:
        model = SimilarityModel()
        model.fit(csv_path)
        similar = model.get_similar_players("COOPER, T.", n=5)
        diff    = model.compare_players("COOPER, T.", "MONACCHI, T.")
    """

    def __init__(self):
        self._feat_df:  Optional[pd.DataFrame] = None   # features crudas
        self._norm_df:  Optional[pd.DataFrame] = None   # features normalizadas
        self._wvecs:    Optional[np.ndarray]   = None   # vectores ponderados
        self._params:   Optional[dict]         = None   # media/std por feature
    ...
    def _find_player(self, player_name: str, season: Optional[str] = None) -> int:
        ...
        # Si hay varias temporadas, tomar la última
    ...
    def get_similar_players(
        ...
        # Filtrar misma temporada, excluir al propio jugador
        ...
        # Excluir al jugador de referencia
