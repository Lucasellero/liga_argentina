// ── Auth guard (acceso restringido: requiere formativas_access:true en metadata) ──
(function() {
  const LOGIN_URL = '../login.html?returnTo=argentina_formativas/';
  const token = localStorage.getItem('auth_token');

  function showDenied() {
    const ov = document.getElementById('accessDenied');
...
function switchSection(id) {
  ...
  if(id==='j-tiro') {
    ...
    if (main && main.style.display !== 'block') {
      ...
      const idx = PLAYERS.findIndex(p => _norm(p.Equipo).includes('ARG'));
...
function renderShotMap() {
  ...
  // FIBA half-court canvas (shots are pre-normalized to attack same basket)
  const W = canvas.offsetWidth || 600;
  const H = Math.round(W * FIBA_H / FIBA_W);
...
function drawCourt(ctx, W, H) {
  // FIBA half-court. Canvas: W × H where H = W * (FIBA_H / FIBA_W).
  // Scale: S = W / FIBA_W pixels per FIBA unit (uniform x/y).
  const S   = W / FIBA_W;
  const p   = u => u * S;  // FIBA unit → canvas pixel (same scale for x and y)

  const bxp = p(FIBA_BX), byp = p(FIBA_BY);   // basket in canvas pixels
  ...
  // Court boundary (endline at top y=0, sidelines, half-court line at bottom)
  ctx.strokeRect(0.5, 0.5, W - 1, H - 1);

  // Paint rectangle: from endline (y=0) to FT line (y=FIBA_PD)
  ctx.strokeRect(p(FIBA_BX - FIBA_PW2), 0, p(FIBA_PW2 * 2), p(FIBA_PD));

  // FT circle: center at (bxp, p(FIBA_PD)), radius = p(FIBA_FTR)
  // Solid half facing away from endline (lower arc), dashed half toward endline
  ctx.beginPath();
  ctx.arc(bxp, p(FIBA_PD), p(FIBA_FTR), 0, Math.PI, false); // solid: bottom half
  ctx.stroke();
  ctx.save(); ctx.setLineDash([Math.max(3, lw * 3), Math.max(3, lw * 3)]);
  ctx.beginPath();
  ctx.arc(bxp, p(FIBA_PD), p(FIBA_FTR), Math.PI, Math.PI * 2, false); // dashed: top half
  ctx.stroke(); ctx.restore();

  // Restricted area arc: center at basket, semi-circle facing away from endline
  ctx.beginPath();
  ctx.arc(bxp, byp, p(FIBA_RAR), 0, Math.PI, false);
  ctx.stroke();

  // Basket ring
  ctx.strokeStyle = 'rgba(251,146,60,.7)';
  ctx.beginPath(); ctx.arc(bxp, byp, p(5.8), 0, Math.PI * 2); ctx.stroke();
  ctx.strokeStyle = lc;

  // 3pt line: two corner straight segments + arc
  const jDY    = Math.sqrt(FIBA_R3 * FIBA_R3 - (FIBA_BX - FIBA_CX) ** 2);
  const jY     = FIBA_BY + jDY;
  const cxLp   = p(FIBA_CX),            cxRp = p(FIBA_W - FIBA_CX);
  const jYp    = p(jY);

  // Corner straight lines
  ctx.beginPath(); ctx.moveTo(cxLp, 0); ctx.lineTo(cxLp, jYp); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cxRp, 0); ctx.lineTo(cxRp, jYp); ctx.stroke();

  // Arc from left junction to right junction (clockwise in screen = through top of key)
...
function drawShots(ctx, W, H, shots, local, visit) {
  ...
  shots.forEach(s => {
    const fx = parseFloat(s['x']), fy = parseFloat(s['y']);
    if (isNaN(fx) || isNaN(fy) || (fx === 0 && fy === 0)) return;
    const x = fx * S, y = fy * S;
...
const FIBA_PW2 = 46;    // half-width of paint (≈2.45m)
const FIBA_PD  = 109;   // paint depth from endline (≈5.8m, FT line y)
const FIBA_FTR = 34;    // FT circle radius (≈1.8m)
const FIBA_RAR = 23;    // restricted area radius (≈1.25m)
const FIBA_CX  = 17;    // corner x boundary (≈0.9m from sideline)

// Zone label centers in FIBA units [x, y]
const SZC_CENTERS = {
  PAINT:       [140,  75],   // center of paint area
  MID_TOP:     [ 70,  85],   // left elbow / left wing mid-range
  MID_CENTER:  [140, 120],   // top of key mid-range
...
function szcClassifyCoord(col, row, W, H) {
  // Convert canvas pixel to FIBA units (basket at FIBA_BX, FIBA_BY)
  const fx = col / W * FIBA_W;
  const fy = row / H * FIBA_H;
  if (fx < 0 || fx > FIBA_W || fy < 0 || fy > FIBA_H) return null;
  const dx = fx - FIBA_BX, dy = fy - FIBA_BY;

  // Corner sideline band (straight part of 3pt line, x < FIBA_CX or x > FIBA_W-FIBA_CX)
  if (fx < FIBA_CX)              return dy < -dx ? 'CORNER_TOP' : 'ABOVE_BREAK';
  if (fx > FIBA_W - FIBA_CX)    return dy <  dx ? 'CORNER_BOT' : 'ABOVE_BREAK';
  ...
  // 2pt territory
  if (fy <= FIBA_PD && Math.abs(dx) <= FIBA_PW2) return 'PAINT';
  if (dist <= FIBA_RAR) return 'PAINT';
  // Mid-range: split left/right by paint edges
  if (dx < -FIBA_PW2) return 'MID_TOP';    // left wing/elbow
  if (dx >  FIBA_PW2) return 'MID_BOT';    // right wing/elbow
  return 'MID_CENTER';                       // top of key
}
...
function szcClassifyShot(s) {
  ...
  const fx = parseFloat(s['x']);
  const fy = parseFloat(s['y']);
  // FT and missing coords default to paint/above-break
  if (isNaN(fx) || isNaN(fy) || (fx === 0 && fy === 0)) {
  ...
  if (is3) {
    ...
    if (fy <= FIBA_PD && Math.abs(dx) <= FIBA_PW2) return 'PAINT';
    if (dist <= FIBA_RAR) return 'PAINT';
    if (dx < -FIBA_PW2) return 'MID_TOP';
    if (dx >  FIBA_PW2) return 'MID_BOT';
    return 'MID_CENTER';
  }
}
...
function szcDrawZoneColors(ctx, W, H, _unused, playerStats, leagueStats) {
  const img = ctx.createImageData(W, H);
  const d = img.data;
  for (let row = 0; row < H; row++) {
    for (let col = 0; col < W; col++) {
      const zone = szcClassifyCoord(col, row, W, H);
      const c = zone
...
function szcUpdateSvg(pStats, leagueStats, svgId='szcSvg') {
  ...
  // FIBA half-court SVG in FIBA units (viewBox="0 0 280 175")
  const bx = FIBA_BX, by = FIBA_BY, R3 = FIBA_R3;
  const pw2 = FIBA_PW2, pd = FIBA_PD, ftr = FIBA_FTR, rar = FIBA_RAR, cx_line = FIBA_CX;
  const f = n => n.toFixed(2);

  // 3pt arc junction y (where corner straight line meets the arc)
  const jDY = Math.sqrt(R3*R3 - (bx - cx_line)**2); // ≈ 62.6
  const jY  = by + jDY;  // ≈ 94.6

  // Mid-range separators: vertical lines at paint edges from FT line to 3pt arc
  const midArcDY_L = Math.sqrt(R3*R3 - pw2**2);   // dy at x = bx ± pw2
  const midArcY    = by + midArcDY_L;              // ≈ 162.8

  // Diagonal separators (45° lines between corner and above-break)
  // Left diag: dy = -dx → y = by - (x-bx) = 172-x. Arc intersection at u = R3/√2 ≈ 97.6:
  //   x = bx - R3/√2 ≈ 42.4, y = by + R3/√2 ≈ 129.6
  const diagU  = R3 / Math.SQRT2;
  const dArcXL = bx - diagU, dArcYL = by + diagU;  // left diag / arc intersection
  const dArcXR = bx + diagU, dArcYR = by + diagU;  // right diag / arc intersection
  // Edge: left diag hits x=0 at y = by - (0-bx) = by+bx = 172; right hits x=280 at y = 172
  const dEdgeY = by + bx;  // = 172
  ...
  let defs = `<defs>
    <filter id="lblShadow" x="-60%" y="-60%" width="220%" height="220%">
      <feDropShadow dx="0" dy="2" stdDeviation="4" flood-color="rgba(0,0,0,.85)"/>
    </filter>`;
  let labels = '';
  SZC_ZONES.forEach(zone => {
    ...
    const rw = 20, rh = 11.5, rx = 4;
    labels += `<g filter="url(#lblShadow)">
      <rect x="${cx-rw/2}" y="${cy-rh/2}" width="${rw}" height="${rh}" rx="${rx}" fill="rgba(6,8,22,.92)"/>
      <rect x="${cx-rw/2}" y="${cy-rh/2}" width="${rw}" height="${rh}" rx="${rx}" fill="url(#lbg_${zone})"/>
      <rect x="${cx-rw/2}" y="${cy-rh/2}" width="${rw}" height="${rh}" rx="${rx}" fill="none" stroke="${bc}" stroke-width="0.6"/>
      <text x="${cx}" y="${cy-1}" text-anchor="middle" dominant-baseline="auto" font-family="Inter,sans-serif" font-size="5.8" font-weight="800" fill="#fff">${pct}</text>
      <text x="${cx}" y="${cy+5.5}" text-anchor="middle" dominant-baseline="auto" font-family="Inter,sans-serif" font-size="3.8" font-weight="600" fill="rgba(200,212,228,.8)">${att}</text>
    </g>`;
  });
  defs += '</defs>';

  // Paint: from endline (y=0) to FT line (y=pd), width ±pw2
  // FT circle: center (bx, pd), radius ftr — solid half facing court, dashed half toward endline
  // RA arc: center (bx, by), radius rar — semi-circle facing court (y>by)
  // 3pt arc: corners at x=cx_line (left) and x=W-cx_line (right), junction at y=jY
  svg.setAttribute('viewBox', `0 0 ${FIBA_W} ${FIBA_H}`);

  svg.innerHTML = `
    ${defs}
    <!-- Court boundary -->
    <rect x="0.5" y="0.5" width="${FIBA_W-1}" height="${FIBA_H-1}" fill="none" stroke="${lc}" stroke-width="${sw}"/>
    <!-- Paint -->
    <rect x="${bx-pw2}" y="0" width="${pw2*2}" height="${pd}" fill="none" stroke="${lc}" stroke-width="${sw}"/>
    <!-- FT circle: solid arc away from endline, dashed toward endline -->
    <path d="M ${bx-ftr} ${pd} A ${ftr} ${ftr} 0 0 0 ${bx+ftr} ${pd}" fill="none" stroke="${lc}" stroke-width="${sw}"/>
    <path d="M ${bx+ftr} ${pd} A ${ftr} ${ftr} 0 0 0 ${bx-ftr} ${pd}" fill="none" stroke="${lc}" stroke-width="${sw}" stroke-dasharray="6 6"/>
    <!-- Restricted area arc -->
    <path d="M ${bx-rar} ${by} A ${rar} ${rar} 0 0 0 ${bx+rar} ${by}" fill="none" stroke="${lc}" stroke-width="${sw}"/>
    <!-- Basket ring -->
    <circle cx="${bx}" cy="${by}" r="5.8" fill="none" stroke="rgba(251,146,60,.95)" stroke-width="${sw*1.5}"/>
    <!-- 3pt corner lines -->
    ...
    <line x1="${bx+pw2}" y1="${pd}" x2="${bx+pw2}" y2="${f(midArcY)}" stroke="${sep}" stroke-width="${sw}" stroke-dasharray="5 5"/>
    <!-- Zone separators (dashed): 45° diagonals separating corners from above-break -->
    <line x1="${f(dArcXL)}" y1="${f(dArcYL)}" x2="0" y2="${f(dEdgeY)}" stroke="${sep}" stroke-width="${sw}" stroke-dasharray="5 5"/>
    <line x1="${f(dArcXR)}" y1="${f(dArcYR)}" x2="${FIBA_W}" y2="${f(dEdgeY)}" stroke="${sep}" stroke-width="${sw}" stroke-dasharray="5 5"/>
    ${labels}
  `;
}
...
function renderZoneChart(canvas, playerShots) {
  ...
  if (svg) { svg.style.height = H + 'px'; svg.setAttribute('viewBox', `0 0 ${FIBA_W} ${FIBA_H}`); }
  const ctx = canvas.getContext('2d');
  const m = null; // unused in FIBA mode
...
function selectSzcPlayer(idx) {
  ...
  const doRender = () => {
    ...
    const gidSet = szcPlayerGameIds ? new Set(szcPlayerGameIds) : null;
    const allShots = gidSet ? rawShots.filter(s => gidSet.has(s['IdPartido'])) : rawShots;
    ...
    PBP_MAP.forEach(events => {
      ...
      sorted.forEach((ev, i) => {
        ...
        for (let j = i - 1; j >= Math.max(0, i - 5); j--) {
          const prev = sorted[j];
          if ((prev['Tipo'] === 'CANASTA-1P' || prev['Tipo'] === 'CANASTA-2P' || prev['Tipo'] === 'CANASTA-3P') && prev['Equipo_lado'] === side) {
