#!/usr/bin/env python3
"""
Liga Nacional Basketball - Play-by-Play Scraper

Scrapes jugada-a-jugada data from the "En vivo" section.

Usage:
    python pbp_scraper_nacional.py               # Scrape all new games
    python pbp_scraper_nacional.py --full        # Re-scrape everything
    python pbp_scraper_nacional.py --dry-run     # List games, no fetch
    python pbp_scraper_nacional.py --output path.csv
"""
...
OUTPUT_CSV = DOCS_DIR / "liga_nacional_pbp.csv"
...
BLOCKED_GAME_KEYS: set[tuple[str, frozenset[str]]] = {
    ("05/03/2026", frozenset({"BOCA", "INSTITUTO"})),  # Supercopa (cancha Atenas)
}
...
def _parse_li(li) -> dict:
    ...
    if is_boundary:
        # Extract period from titulo span for INICIO/FINAL-PERIODO
    ...
    if score_el:
        score_text = score_el.get_text(strip=True)  # "121 - 118"
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Liga Nacional play-by-play scraper")
    ...
    # (los IDs del sitio son dinámicos y cambian en cada request — ver CLAUDE.md
    # "IDs dinámicos del sitio". Usar solo la fila LOCAL evita duplicar el partido
    # bajo dos IdPartido distintos, uno por cada lado.)
    ...
    # Cache: clave estable fecha|equipo_local|equipo_visitante desde el CSV de PBP
    ...
    new_game_keys = [
        k for k in games
        if k not in cached_keys and not _is_blocked(games[k]["fecha"], games[k]["local"], games[k]["visitante"])
    ]
    ...
    # Deduplicar por contenido de la acción (no por IdPartido, que es dinámico)
    before = len(merged)
    merged = merged.drop_duplicates(
        subset=["Fecha", "Equipo_local", "Equipo_visitante", "NumAccion"],
        keep="last",
    )
